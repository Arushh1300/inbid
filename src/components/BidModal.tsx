...
interface ModalLocationDropdownProps {
  ...
  onSelect: (item: ModalDropdownItem) => void;
  isLoading?: boolean;
  allowCustom?: boolean;
...
function ModalLocationDropdown({
  ...
  const selectedItem = items.find((i) => i.name.toLowerCase() === (value || '').toLowerCase());
      ...
      {isCustomActive ? (
        ...
        <>
          <button
            ...
            className={`w-full bg-slate-50 border rounded-xl px-2.5 py-2 text-xs font-extrabold transition-all flex items-center justify-between cursor-pointer group text-left ${
              isOpen
            ...
            <div className="space-y-2 pt-1 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <ModalLocationDropdown
                  ...
                  onSelect={(item) => handleCountryChange(item.name)}
                />
                ...
                <ModalLocationDropdown
                  ...
                  onSelect={(item) => handleStateChange(item.name)}
                />
                ...
                <ModalLocationDropdown
                  ...
                  items={citiesList.map((ct) => ({ name: ct }))}
                  allowCustom={true}
